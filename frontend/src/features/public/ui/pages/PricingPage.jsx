...
const PricingPage = () => {
  ...
  const { isAuthenticated, user, handleUpdatePlan } = useAuth();
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20 items-stretch">
        {plans.map((plan) => {
          ...
          const isCurrentPlan = isAuthenticated && (user?.plan || "free") === plan.id;

          let buttonLabel = plan.buttonText;
          if (isAuthenticated) {
            if (plan.id === "enterprise") {
              buttonLabel = isCurrentPlan ? "Active Dedicated VPC" : "Contact Enterprise Sales";
            } else if (isCurrentPlan) {
              buttonLabel = "Current Active Plan";
            } else if (plan.id === "pro") {
              buttonLabel = "Upgrade to Pro Cloud ($19/mo)";
            } else if (plan.id === "free") {
              buttonLabel = "Downgrade to Free Sandbox";
            }
          }
          ...
          return (
            <div
              ...
              className={`relative bg-white dark:bg-slate-800/90 rounded-3xl p-8 border transition-all duration-200 cursor-pointer flex flex-col justify-between h-full ${
                isPopular
                  ? "border-[#1a73e8] shadow-lg ring-2 ring-[#1a73e8]/20"
                  : "border-slate-200 dark:border-slate-700 shadow-xs hover:shadow-md hover:border-slate-300 dark:hover:border-slate-600"
              }`}
            >
              {isPopular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-0.5 rounded-full text-xs font-bold bg-[#1a73e8] text-white shadow-xs whitespace-nowrap">
                  Most Popular
                </div>
              )}
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex items-center justify-between gap-2 mb-3 h-8">
                  ...
                  <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 whitespace-nowrap shrink-0">
                    {plan.badge}
                  </span>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 leading-relaxed min-h-[36px] line-clamp-2">
                  {plan.description}
                </p>

                <div className="flex items-baseline gap-1.5 mb-6 h-10">
                ...
                <ul className="space-y-3 pt-6 border-t border-slate-100 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-200 flex-1">
