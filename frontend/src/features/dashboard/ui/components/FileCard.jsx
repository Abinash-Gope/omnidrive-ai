...
const FileCard = ({ file, onOpenPreview, viewMode = "grid" }) => {
  ...
  if (viewMode === "list") {
    return (
      ...
      >
        ...
        {/* Right: Meta & Actions */}
        <div className="flex items-center gap-6 shrink-0 text-xs text-slate-500 dark:text-slate-400">
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex items-start justify-between gap-2">
            <h3
              className="text-sm font-semibold text-slate-900 dark:text-white truncate group-hover:text-[#1a73e8] transition-colors"
              title={file.name}
            >
              {file.name}
            </h3>
